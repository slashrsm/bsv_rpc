defmodule BsvRpcTest do
  ...
  test_with_mock "genserver is called for get_memory_info", _context, GenServer, [],
    ...
    {:ok, info} = BsvRpc.get_memory_info()
  ...
  test_with_mock "genserver is called for uptime", _context, GenServer, [],
    call: fn _module, _context -> {:ok, 158_535} end do
    assert BsvRpc.uptime() == {:ok, 158_535}
    assert called(GenServer.call(BsvRpc, {:call_endpoint, "uptime"}))
  end

  test_with_mock "genserver is called for stop", _context, GenServer, [],
    call: fn _module, _context -> {:ok, "Bitcoin server stopping"} end do
    assert BsvRpc.stop() == {:ok, "Bitcoin server stopping"}
  ...
  test_with_mock "genserver is called for get_wallet_info", _context, GenServer, [],
    ...
    {:ok, info} = BsvRpc.get_wallet_info()
    assert info["balance"] == 0.0
  ...
  test_with_mock "genserver is called for list_accounts", _context, GenServer, [],
    call: fn _module, _context -> {:ok, %{"account_1" => 0.0}} end do
    {:ok, account} = BsvRpc.list_accounts(3, true)
    assert account["account_1"] == 0.0
  ...
  test_with_mock "default params for list_accounts", _context, GenServer, [],
    call: fn _module, _context -> {:ok, %{"account_1" => 0.0}} end do
    {:ok, account} = BsvRpc.list_accounts()
    assert account["account_1"] == 0.0
  ...
  test_with_mock "genserver is called for get_balance", _context, GenServer, [],
    call: fn _module, _context -> {:ok, 123.456} end do
    assert BsvRpc.get_balance("foo", 5, true) == {:ok, 123.456}
  ...
  test_with_mock "default params for get_balance", _context, GenServer, [],
    call: fn _module, _context -> {:ok, 123.456} end do
    assert BsvRpc.get_balance() == {:ok, 123.456}
  ...
  test_with_mock "genserver is called for get_unconfirmed_balance", _context, GenServer, [],
    call: fn _module, _context -> {:ok, 123.456} end do
    assert BsvRpc.get_unconfirmed_balance() == {:ok, 123.456}
  ...
  test_with_mock "genserver is called for get_new_address", _context, GenServer, [],
    call: fn _module, _context -> {:ok, "1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62i"} end do
    assert {:ok,
            %BsvRpc.Address{
  ...
  test_with_mock "error handling on get_new_address", _context, GenServer, [],
    call: fn _module, _context -> {:error, "Who would know?"} end do
    assert {:error, "Who would know?"} == BsvRpc.get_new_address("foo")
    assert called(GenServer.call(BsvRpc, {:call_endpoint, "getnewaddress", ["foo"]}))
  end
  ...
  test_with_mock "error handling on get_new_address with default params", _context, GenServer, [],
    call: fn _module, _context -> {:error, "Who would know?"} end do
    assert {:error, "Who would know?"} == BsvRpc.get_new_address()
    assert called(GenServer.call(BsvRpc, {:call_endpoint, "getnewaddress"}))
  end

  test_with_mock "genserver is called for get_addresses_by_account", _context, GenServer, [],
    call: fn _module, _context ->
      {:ok, ["1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62i", "1Ax4gZtb7gAit2TivwejZHYtNNLT18PUXJ"]}
    end do
    assert {:ok,
            [
              %BsvRpc.Address{
  ...
  test_with_mock "error handling on get_addresses_by_account", _context, GenServer, [],
    call: fn _module, _context -> {:error, "Who would know?"} end do
    assert {:error, "Who would know?"} == BsvRpc.get_addresses_by_account("foo")
    assert called(GenServer.call(BsvRpc, {:call_endpoint, "getaddressesbyaccount", ["foo"]}))
  end
  ...
  test_with_mock "error handling on get_addresses", _context, GenServer, [],
    call: fn _module, _context -> {:error, "Who would know?"} end do
    assert {:error, "Who would know?"} == BsvRpc.get_addresses()
    assert called(GenServer.call(BsvRpc, {:call_endpoint, "getaddressesbyaccount", [""]}))
  end
  ...
  test_with_mock "genserver is called for get_transaction", _context, GenServer, [],
    ...
    {:ok, t} =
      BsvRpc.get_transaction("2de56313f760c3a81cfbac22e7cc34958c2a7a8d6c739aa8861f65e42692669d")
  ...
  test_with_mock "error handling on get_transaction", _context, GenServer, [],
    call: fn _module, _context -> {:error, "Who would know?"} end do
    assert {:error, "Who would know?"} ==
             BsvRpc.get_transaction(
               "2de56313f760c3a81cfbac22e7cc34958c2a7a8d6c739aa8861f65e42692669d"
             )
  ...
  test_with_mock "error handling on list_unspent", _context, GenServer, [],
    call: fn _module, _context -> {:error, "Who would know?"} end do
    assert {:error, "Who would know?"} ==
             BsvRpc.list_unspent(
               [BsvRpc.Address.create!("1LNEsmx2nr4jtsZqWSpuGBUXngadosczf7")],
  ...
  test_with_mock "error handling on sign_transaction", _context, GenServer, [],
    call: fn _module, _context -> {:error, "Who would know?"} end do
    assert {:error, "Who would know?"} ==
             BsvRpc.Transaction.create_from_hex(
               ""
  ...
  test_with_mock "error handling on broadcast_transaction", _context, GenServer, [],
    call: fn _module, _context -> {:error, "Who would know?"} end do
    assert {:error, "Who would know?"} ==
             BsvRpc.Transaction.create_from_hex(
               ""
